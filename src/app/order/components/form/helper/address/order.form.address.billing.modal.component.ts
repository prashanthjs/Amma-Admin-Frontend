...
@Component({
  selector: 'app-order-form-address-billing-modal',
  templateUrl: './order.form.address.billing.modal.template.html'
})
export class OrderFormAddressBillingModalComponent implements OnInit, OnDestroy {
  ...
  constructor(public modalRef: BsModalRef,
    ...
    this.formService.transferModel = (result: any): Object => {
      return ObjectPath.get(result, 'address.billing', {});
    };

    this.formService.transferSubmitModel = (result: any): Object => {
      ObjectPath.set(this.formService.result, 'address.billing', result);
