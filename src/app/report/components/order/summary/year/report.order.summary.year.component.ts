...
import {
  CHART_BAR_CURRENCY_OPTIONS, CHART_BAR_NORMAL_OPTIONS, CHART_LINE_COLORS,
  CHART_LINE_COLORS1
...
@Component({
  selector: 'app-report-order-summary-year',
  templateUrl: './report.order.summary.year.template.html'
})
export class ReportOrderSummaryYearComponent implements OnInit, OnDestroy {
  ...
  getSummary() {
    ...
    this.chartLabel = [];
    this.service.getYearSummary(listOptions).subscribe((result: any) => {
      const countData = [];
      const amountData = [];
      this.result = result;
      ...
      this.chartCountData = [{data: countData, label: 'Annually'}];
      this.chartAmountData = [{data: amountData, label: 'Annually'}];
    });
  }
